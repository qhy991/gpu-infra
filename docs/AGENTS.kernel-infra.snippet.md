# 可复制到下游项目 `AGENTS.md` 的 Kernel Infra 约定

将下面代码块复制到需要通过 Agent 进行 kernel 开发和 GPU 评测的项目中。把 `/absolute/path/to/gpu-infra` 替换为本机实际路径；若 `$kernel-infra` 已安装，则无需固定本地路径。

```markdown
## GPU kernel 评测

- 涉及 kernel 正确性、性能测试、A800/B200、PTXBench、FIBServe、KDA 或 GPU 队列时，使用 `$kernel-infra`。若该 Skill 尚未安装，先读取 `/absolute/path/to/gpu-infra/skills/kernel-infra/SKILL.md`，并以当前 checkout 的 `AGENTS.md`、CLI 和 task 合同为准。
...
完整操作命令和证据语义由 [`../skills/kernel-infra/SKILL.md`](../skills/kernel-infra/SKILL.md) 维护，本片段不复制具体远程主机、socket、GPU 编号或 task 参数。
